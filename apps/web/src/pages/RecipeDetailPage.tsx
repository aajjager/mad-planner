...
export function RecipeDetailPage() {
  ...
  return <article className="page recipe-detail">
    ...
    <section className="meal-classification"><div><h2>Suitable for</h2><p>Choose every meal where this recipe belongs.</p></div><div className="choice-chips">{(['breakfast', 'lunch', 'dinner'] as RecipeMealType[]).map((mealType) => <button type="button" className={`choice-chip${(recipe.meal_types || []).includes(mealType) ? ' choice-chip--selected' : ''}`} aria-pressed={(recipe.meal_types || []).includes(mealType)} onClick={() => toggleMealType(mealType)} key={mealType}>{mealType}</button>)}</div></section>
