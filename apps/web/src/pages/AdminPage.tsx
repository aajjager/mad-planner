...
export function AdminPage() {
  ...
  return <section className="page admin-page">
    ...
    <div className="admin-stack">
      {account.is_system_admin && <section className="family-panel"><h2>Backups</h2><p>Download the database and uploaded recipe photos separately. Keep both files together for a complete recovery copy.</p><div className="heading-actions"><button className="button button--primary" disabled={Boolean(busy)} onClick={async () => { setBusy('backup'); setError(''); try { await downloadDatabaseBackup() } catch (reason) { setError(reason instanceof Error ? reason.message : t('requestFailed')) } finally { setBusy('') } }}>{busy === 'backup' ? 'Creating backup…' : 'Download database'}</button><button className="button" disabled={Boolean(busy)} onClick={async () => { setBusy('media-backup'); setError(''); try { await downloadMediaBackup() } catch (reason) { setError(reason instanceof Error ? reason.message : t('requestFailed')) } finally { setBusy('') } }}>{busy === 'media-backup' ? 'Creating photo backup…' : 'Download recipe photos'}</button></div></section>}
