...
export function AdminPage() {
  ...
  return <section className="page admin-page">
    ...
    <div className="admin-stack">
      {account.is_system_admin && <section className="family-panel"><h2>Database backup</h2><p>Download a portable backup of accounts, families, recipes, plans, and grocery data. Uploaded photos are backed up separately.</p><button className="button button--primary" disabled={Boolean(busy)} onClick={async () => { setBusy('backup'); setError(''); try { await downloadDatabaseBackup() } catch (reason) { setError(reason instanceof Error ? reason.message : t('requestFailed')) } finally { setBusy('') } }}>{busy === 'backup' ? 'Creating backup…' : 'Download backup'}</button></section>}
