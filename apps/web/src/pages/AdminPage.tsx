...
export function AdminPage() {
  ...
  async function completeApprovedGroup(category: FeedbackCategory) {
    const count = feedback.filter((item) => item.status === 'approved' && item.category === category).length
    if (!window.confirm(t('markApprovedGroupDoneConfirm').replace('{count}', String(count)))) return
    setBusy(`feedback-group-${category}`); setError('')
    try { await completeApprovedFeedback(category); await refresh() }
    catch (reason) { setError(reason instanceof Error ? reason.message : t('requestFailed')) }
    finally { setBusy('') }
  }
  ...
  return <section className="page admin-page">
    ...
    <div className="admin-stack">
      ...
      {account.is_system_admin && <section className="family-panel"><h2>{t('copyApprovedBatches')}</h2><p>{t('copyApprovedBatchesHelp')}</p><div className="feedback-review-list">{(['improvement', 'bug', 'feature'] as FeedbackCategory[]).map((category) => { const count = feedback.filter((item) => item.status === 'approved' && item.category === category).length; return <article key={category}><header><strong>{t(category === 'feature' ? 'featureRequest' : category)}</strong><span className="feedback-status feedback-status--approved">{count} {t('approved')}</span></header><footer><button className="button" disabled={count === 0 || Boolean(busy)} onClick={() => void copyApprovedGroup(category)}>{copiedGroup === category ? t('copiedForCodex') : t('copyGroupForCodex')}</button><button className="button button--primary" disabled={count === 0 || Boolean(busy)} onClick={() => void completeApprovedGroup(category)}>{t('markApprovedGroupDone')}</button></footer></article> })}</div></section>}
