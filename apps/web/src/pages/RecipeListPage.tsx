...
import { bulkUpdateRecipes, listRecipes, type Recipe } from '../api/recipes'
...
export function RecipeListPage() {
  ...
  const [selecting, setSelecting] = useState(false); const [selected, setSelected] = useState<number[]>([]); const [bulkTags, setBulkTags] = useState(''); const [bulkBusy, setBulkBusy] = useState(false); const [bulkError, setBulkError] = useState('')
  ...
  const chosenTags = () => bulkTags.split(',').map((value) => value.trim()).filter(Boolean)
  async function applyBulk(change: { is_public?: boolean; add_tags?: string[]; remove_tags?: string[] }) { setBulkBusy(true); setBulkError(''); try { const updated = await bulkUpdateRecipes({ recipe_ids: selected, ...change }); const byId = new Map(updated.map((recipe) => [recipe.id, recipe])); setRecipes((current) => current.map((recipe) => byId.get(recipe.id) || recipe)); setBulkTags('') } catch (reason) { setBulkError(reason instanceof Error ? reason.message : t('bulkUpdateFailed')) } finally { setBulkBusy(false) } }
  function toggleSelected(id: number) { setSelected((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id]) }

  return <section className="page">
    <div className="page-heading"><div><p className="eyebrow">{t('collection')}</p><h1>{t('yourRecipes')}</h1><p>{t('recipeIntro')}</p></div><div className="heading-actions">{canEdit && <button className="button" onClick={() => { setSelecting((value) => !value); setSelected([]) }}>{selecting ? t('finishSelecting') : t('selectRecipes')}</button>}<Link className="button" to="/recipes/public">{t('browsePublicRecipes')}</Link><Link className="button" to="/recipes/scan">{t('scanBook')}</Link><Link className="button" to="/recipes/import/cookbook">{t('importCookBook')}</Link><Link className="button" to="/recipes/import">{t('importUrl')}</Link><Link className="button button--primary" to="/recipes/new">{t('addRecipe')}</Link></div></div>
    ...
    {selecting && <section className="bulk-recipe-actions"><strong>{selected.length} {t('selected')}</strong><button className="button" disabled={bulkBusy} onClick={() => setSelected(filteredRecipes.filter((recipe) => recipe.owned_by_current_family).map((recipe) => recipe.id))}>{t('selectVisible')}</button><button className="button button--primary" disabled={bulkBusy || selected.length === 0} onClick={() => void applyBulk({ is_public: true })}>{t('makePublic')}</button><button className="button" disabled={bulkBusy || selected.length === 0} onClick={() => void applyBulk({ is_public: false })}>{t('makePrivate')}</button><input value={bulkTags} placeholder={t('tagsPlaceholder')} onChange={(event) => setBulkTags(event.target.value)} /><button className="button" disabled={bulkBusy || selected.length === 0 || chosenTags().length === 0} onClick={() => void applyBulk({ add_tags: chosenTags() })}>{t('addTags')}</button><button className="button button--danger" disabled={bulkBusy || selected.length === 0 || chosenTags().length === 0} onClick={() => void applyBulk({ remove_tags: chosenTags() })}>{t('removeTags')}</button>{bulkError && <span className="notice notice--error">{bulkError}</span>}</section>}
    ...
    {filteredRecipes.length > 0 && <div className="recipe-grid">{filteredRecipes.map((recipe) => <div className={`selectable-recipe${selected.includes(recipe.id) ? ' selectable-recipe--selected' : ''}`} key={recipe.id}>{selecting && recipe.owned_by_current_family && <button className="recipe-selector" aria-pressed={selected.includes(recipe.id)} aria-label={`${t('select')} ${recipe.name}`} onClick={() => toggleSelected(recipe.id)}>{selected.includes(recipe.id) ? '✓' : ''}</button>}<Link className="recipe-card" to={`/recipes/${recipe.id}`}><div className="recipe-card__image">{recipe.image_url ? <img src={recipe.image_url} alt="" /> : <span>M</span>}{(recipe.owned_by_current_family === false || recipe.shared_with_families?.length > 0) && <span className="recipe-share-badge" title={recipe.owned_by_current_family === false ? `${t('sharedBy')} ${recipe.owner_family_name}` : `${t('sharedWith')} ${recipe.shared_with_families.join(', ')}`}><FamilyShareIcon /></span>}</div><div className="recipe-card__body"><div className="recipe-meta">{recipe.category && <span>{recipe.category}</span>}{recipe.total_time_minutes !== null && <span>{recipe.total_time_minutes} min</span>}{recipeCalories(recipe) !== null && <span>{Math.round(recipeCalories(recipe)!)} kcal</span>}</div><h2>{recipe.name}</h2>{recipe.family_rating != null && <RatingStars value={null} average={recipe.family_rating} count={recipe.rating_count} label={t('familyRating')} />}<p>{recipe.description || t('openRecipe')}</p>{recipe.tags.length > 0 && <div className="tag-list">{recipe.tags.slice(0, 3).map((item) => <span className="tag" key={item}>{item}</span>)}</div>}</div></Link></div>)}</div>}
  </section>
}
