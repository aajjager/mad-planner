...
import { createRecipe, type RecipeIngredientInput, type RecipeMealType, type RecipeWrite, type UnitDimension } from '../api/recipes'
...
export function CreateRecipePage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: RecipeWrite = { name: String(form.get('name')).trim(), description: text('description'), source_url: text('source_url'), author: text('author'), servings: text('servings'), cuisine: text('cuisine'), category: text('category'), tags, meal_types: selectedMealTypes, preparation_time_minutes: minutes('preparation'), cooking_time_minutes: minutes('cooking'), total_time_minutes: minutes('total'), ingredients: items, instructions: instructions.filter((step) => step.trim()).map((step) => ({ text: step.trim() })) }
