...
import { acknowledgeFeedback, createFamilyInvitation, createRecipeType, deleteRecipeType, getFamilySettings, listFamilyMembers, listMyFeedback, listRecipeTypes, submitFeedback, updateFamilySettings, uploadFeedbackAttachment, type FamilyMember, type FamilyRole, type FamilySettings, type Feedback, type RecipeType } from '../api/auth'
...
export function FamilyPage() {
  ...
  async function sendFeedback(event: FormEvent) {
    ...
    try { let created = await submitFeedback(feedback); if (feedbackFile) created = await uploadFeedbackAttachment(created.id, feedbackFile); setMyFeedback((current) => [created, ...current]); setFeedback(''); setFeedbackFile(null); setFeedbackSent(true) }
    catch (reason) { setError(reason instanceof Error ? reason.message : t('requestFailed')) }
    finally { setSubmitting(false) }
  }
  ...
  return <section className="page family-page">
    ...
    <Link className="family-panel updates-settings-link" to="/updates"><span><strong>{t('whatsNew')}</strong><small>{t('updatesIntro')}</small></span><b aria-hidden="true">v0.1.0 →</b></Link>
    <MfaSettings />
    <section className="family-panel feedback-panel"><h2>{t('feedbackTitle')}</h2><p>{t('feedbackHelp')}</p>{myFeedback.filter((item) => item.status === 'done' && !item.completion_seen_at).map((item) => <div className="notice notice--success feedback-complete" role="status" key={item.id}><div><strong>{t('feedbackCompleted')}</strong><p>{item.content}</p></div><button className="button" onClick={() => void dismissCompletion(item)}>{t('dismiss')}</button></div>)}<form onSubmit={sendFeedback}><label className="field"><span>{t('feedbackLabel')}</span><textarea required minLength={5} maxLength={4000} rows={5} value={feedback} onChange={(event) => { setFeedback(event.target.value); setFeedbackSent(false) }} placeholder={t('feedbackPlaceholder')} /></label><label className="field"><span>{t('optionalAttachment')}</span><input type="file" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain" onChange={(event) => setFeedbackFile(event.target.files?.[0] || null)} /></label><button className="button button--primary" disabled={submitting || feedback.trim().length < 5}>{submitting ? t('sending') : t('sendFeedback')}</button>{feedbackSent && <span className="settings-saved" role="status">{t('feedbackSent')}</span>}</form>{myFeedback.length > 0 && <details className="feedback-history"><summary>{t('yourFeedbackHistory')}</summary>{myFeedback.map((item) => <article key={item.id}><span className={`feedback-status feedback-status--${item.status}`}>{t(item.status)}</span><p>{item.content}</p>{item.attachment_url && <a href={item.attachment_url} target="_blank" rel="noreferrer">{item.attachment_name || t('attachment')}</a>}</article>)}</details>}</section>
