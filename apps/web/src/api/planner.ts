...
export interface MealSuggestion { meal_date: string; meal_type: MealType; recipe: Pick<Recipe, 'id' | 'name' | 'image_url'>; score: number; reasons: string[]; is_leftover: boolean; source_date: string | null; source_meal_type: MealType | null }
...
export const planLeftovers = (mealDate: string, mealType: MealType, targetDate?: string, targetMealType?: MealType) => { const params = new URLSearchParams(); if (targetDate) params.set('target_date', targetDate); if (targetMealType) params.set('target_meal_type', targetMealType); const query = params.toString(); return request<MealPlanEntry>(`/api/v1/meal-plans/${mealDate}/${mealType}/leftovers${query ? `?${query}` : ''}`, { method: 'POST' }) }
